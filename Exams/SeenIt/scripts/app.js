    ...
    const app = Sammy('#container', function () {
        ...
        this.post('#/register', (ctx) => {
            ...
            if (!/^[A-Za-z]{3,}$/.test(username)) {
                notify.showError('Username should be at least 3 characters and contain only letters!');
            } else if (!/^[A-Za-z\d]{6,}$/.test(password)) {
                notify.showError('Password should be at least 6 characters and contain only letters and digits!');
        ...
        this.post('#/edit/post',(ctx)=>{
            ...
            if (title === '') {
                ...
                posts.editPost(postId, author, title, description, url, imageUrl)
                    .then(() => {
                        notify.showInfo(`Post ${title} editted.`);
                        ctx.redirect('#/catalog');
                    })
                    .catch(notify.showError);
            }
        ...
        this.get('#/details/:postId', (ctx)=>{
            ...
            const allCommentsPromise= comments.getAllCommentsForPostId(postId);
        ...
        function getWelcomePage(ctx) {
            if (!auth.isAuth()) {
                ...
                ctx.loadPartials({
                    ...
                    this.partial('./templates/welcomeAnonimus.hbs');
                });
            } else {
                ctx.redirect('#/home');
            }
